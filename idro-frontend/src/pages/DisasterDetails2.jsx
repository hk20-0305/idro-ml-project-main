...
export default function DisasterDetails2() {
    ...
    useEffect(() => {
        if (camps.length > 0 && providers.length > 0) {
            console.log("Starting Refined Mission Allocation Logic...");
            const etaRank = {
                'IMMEDIATE': 1,
            ...
            sortedCamps.forEach(camp => {
                ...
                const campAllocationsByProvider = {}; // Track grouped resources per provider
                ...
                resourceKeys.forEach(resKey => {
                    ...
                    viableProviders.forEach(provider => {
                        ...
                        if (amountToTake > 0) {
                            ...
                            provider[resKey] -= amountToTake; // Deplete inventory
    ...
    const toggleAllocationStatus = (id) => {
        setAllocationStatuses(prev => {
            const current = prev[id] || 'ASSIGNED';
            if (current === 'ASSIGNED') return { ...prev, [id]: 'DISPATCHED' };
            if (current === 'DISPATCHED') return { ...prev, [id]: 'DELIVERED' };
            return { ...prev, [id]: 'ASSIGNED' };
        });
    };
                                ...
                                return (
                                    <div key={alloc.id} className="group relative overflow-hidden rounded-[1.5rem] bg-slate-900/40 border-2 border-white/[0.03] backdrop-blur-xl transition-all duration-500 hover:border-blue-500/20 hover:bg-slate-900/60">
                                        ...
                                        <div className="relative z-10 flex flex-col xl:flex-row items-stretch">
                                            ...
                                            <div className="p-5 xl:w-48 bg-black/40 flex flex-col justify-center relative border-t xl:border-t-0 xl:border-l border-white/[0.05]">
                                                ...
                                                <div className="flex items-center gap-2 mt-4">
                                                    ...
                                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Assigned</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
function formatETA(responseTime) {
    if (!responseTime) return 'TBD';
    switch (responseTime.toUpperCase()) {
